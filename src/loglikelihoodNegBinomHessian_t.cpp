/**
 * \file loglikelihoodNegBinomHessian_t.cpp
 * \author Felipe Acosta
 * \date 2015-04-13
 * \brief This function evaluates the LogLikelihood Hessian matrix for the negative binomial
 * regression case with t random effects for the diagonal case up to a constant (the factorial term is ommited.)
 * Arguments:
...
const arma::mat& kX, const arma::mat& kZ) {
  ...
  for (int i = 0; i < kR; i++) {
    ...
    hessian(kP + 1 + i, kP + 1 + i) = 0.5 * kKi(i) / (pow(lambda_i, 2.0)) - 0.5 * (df(i) + kKi(i)) *
