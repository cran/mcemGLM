/**
 * \file MCMCloglikelihoodLogit_t.cpp
...
// [[Rcpp::export]]
arma::vec MCMCloglikelihoodLogitCpp_t(const arma::vec& beta, const arma::mat& sigma, const arma::vec& sigmaType, 
...
const arma::vec& kY, const arma::mat& kX, const arma::mat& kZ) {
  ...
  for (int i = 0; i < kM; i++) {
    loglike(i) = loglikelihoodLogitCpp_t(beta, sigma, sigmaType, u.row(i).t(), df, kKi, kLh, kLhi, kY, kX, kZ);
  }
