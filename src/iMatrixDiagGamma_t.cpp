...
// [[Rcpp::export]]
arma::mat iMatrixDiagGammaCpp_t(const arma::vec& beta, const arma::mat& sigma, double alpha, const arma::vec& sigmaType, const arma::mat& uSample, 
  ...
  for (int i = 0; i < B; i++) {
    g0 = loglikelihoodGammaGradientCpp_t(beta, sigma, alpha, uSample.row(i).t(), df, kKi, kLh, kLhi, kY, kX, kZ);
    h0 = loglikelihoodGammaHessianCpp_t(beta, sigma, alpha, uSample.row(i).t(), df, kKi, kLh, kLhi, kY, kX, kZ);
