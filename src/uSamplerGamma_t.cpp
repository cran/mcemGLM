...
double logAcceptGamma_t(const arma::vec& beta, const arma::mat& sigma, double alpha, const arma::vec& sigmaType, const arma::vec& ucurrent, 
  ...
  return min0(0.0, loglikelihoodGammaCpp_t(beta, sigma, alpha, sigmaType, uproposed, df, kKi, kLh, kLhi, kY, kX, kZ) 
                - loglikelihoodGammaCpp_t(beta, sigma, alpha, sigmaType, ucurrent, df, kKi, kLh, kLhi, kY, kX, kZ));
}
...
arma::mat uSamplerGammaCpp_t(const arma::vec& beta, const arma::mat& sigma, double alpha, const arma::vec& sigmaType, const arma::vec& u, 
  ...
  for (int i = 1; i < B; i++){
    ...
    uproposed += ucurrent;
    if (log(R::runif(0, 1)) < logAcceptGamma_t(beta, sigma, alpha, sigmaType, ucurrent, uproposed, df, kKi, kLh, kLhi, kY, kX, kZ)) {
      ucurrent = uproposed;
    }
