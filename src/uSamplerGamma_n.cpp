...
const arma::vec& uproposed, const arma::vec& kY, const arma::mat& kX, const arma::mat& kZ) {
  return min0(0.0, loglikelihoodGammaCpp_n(beta, sigma, alpha, uproposed, kY, kX, kZ) - loglikelihoodGammaCpp_n(beta, sigma, alpha, ucurrent, kY, kX, kZ));
}

// [[Rcpp::export]]
arma::mat uSamplerGammaCpp_n(const arma::vec& beta, const arma::mat& sigma, double alpha, const arma::vec& u, 
const arma::vec& kY, const arma::mat& kX, const arma::mat& kZ, int B, double sd0) {
  ...
  for (int i = 1; i < B; i++){
    ...
    uproposed += ucurrent;
    if (log(R::runif(0, 1)) < logAcceptGamma_n(beta, sigma, alpha, ucurrent, uproposed, kY, kX, kZ)) {
      ucurrent = uproposed;
    }
