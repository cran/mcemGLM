/**
 * \file loglikelihoodGammaGradient_t.cpp
 * \author Felipe Acosta
 * \date 2015-11-22
 * \brief This function evaluates the LogLikelihood gradient function for the gamma regression 
...
arma::vec loglikelihoodGammaGradientCpp_t(const arma::vec& beta, const arma::mat& sigma, double alpha, const arma::vec& u, 
  ...
  for (int i = 0; i < kR; i++) {
    ...
    gradient(kP + 1 + i) = -0.5 * kKi(i) / lambda_i + 0.5 * (df(i) + kKi(i)) * (sumU / (lambda_i * lambda_i)) / (df(i) + sumU / lambda_i);
  }
