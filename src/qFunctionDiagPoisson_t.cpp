/**
 * \file qFunctionDiagPoisson_t.cpp
...
// [[Rcpp::export]]
List qFunctionDiagPoissonCpp_t(const arma::vec& beta, const arma::mat& sigma, const arma::vec& sigmaType, const arma::mat& u, 
...
const arma::mat& kX, const arma::mat& kZ) {
  ...
  for (int i = 0; i < kM; i++) {
    value += loglikelihoodPoissonCpp_t(beta, sigma, sigmaType, u.row(i).t(), df, kKi, kLh, kLhi, kY, kX, kZ) / kM;
    gradient += loglikelihoodPoissonGradientCpp_t(beta, sigma, u.row(i).t(), df, kKi, kLh, kLhi, kY, kX, kZ) / kM;
    hessian += loglikelihoodPoissonHessianCpp_t(beta, sigma, u.row(i).t(), df, kKi, kLh, kLhi, kY, kX, kZ) / kM;
  }
